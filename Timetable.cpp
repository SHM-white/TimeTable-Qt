...
int TimeTable::sort()
{
	...
	for (const auto& i : Days) {
		if (i == "Null") { continue; }
		Json::Value& vLessons = root[i]["Lessons"];
		auto Lessons = vLessons | std::views::transform([i](const Json::Value& value) {return Lesson(i, value[0].asString(), value[1].asInt(), value[2].asInt()); });
